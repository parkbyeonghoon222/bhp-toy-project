...
export class Dropdown<T = unknown> extends View<DropdownData<T>> {
  ...
  override onMount() {
    ...
    this.addEventListener("focusout", function closeDropdown(e) {
      ...
      if (e.relatedTarget instanceof HTMLElement) {
        ...
        const close_component_el = e.relatedTarget
          ? e.relatedTarget.closest(`.Dropdown`)
          : null;
  ...
  findSelectedOption(selected_option_key?: string) {
    ...
    return (
      this.data.options.find((option) => option.key === selected_option_key) ??
      null
    );
  }
  ...
  override template() {
    ...
    const selected_option = this.findSelectedOption(
      this.data.selected_option_key,
    );
    const horizontal_class = horizontal_class_map[horizontal];
    const vertical_class = vertical_class_map[vertical];

    const dropdown_icon = html`<span class="${c.icon}">
      ${DropdownDownIcon}
    </span>`;

    return html`
      <div
        class="${htmlIf(c.is_opened, is_opened)} ${c.dropdown} ${typo(
          "14_medium",
        )} ${klass}"
      >
        <button class="${c.button} ${button_klass}">
          ${selected_option?.name || default_name}
          ${htmlIf(dropdown_icon, has_arrow)}
        </button>
        <div
          class="${c.option_container} ${horizontal_class} ${vertical_class}"
        >
          ${data.options.map((option) => {
            const selected_class = selected_option == option ? c.selected : "";

            return html` <button
              class="${c.option} ${selected_class}"
              data-key="${option.key}"
            >
              ${option.name}
            </button>`;
